...
template <class T> class ArbolBinarioBusqueda {
private:
  Node<T> *root;
...
public:
  ArbolBinarioBusqueda() { root = NULL; }
  ~ArbolBinarioBusqueda() { delete root; }
  void insertar(int key, T data) { insertar(root, key, data); }
  void imprimir() {
    if (root == NULL) {
  ...
  Node<T> *buscar(int key) { return buscar(root, key); }
};
